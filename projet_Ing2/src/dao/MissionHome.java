...
public class MissionHome {
	...
	public Mission findById(java.math.BigDecimal id) {
		log.debug("getting Mission instance with id: " + id);
		try {
			Mission instance = (Mission) sessionFactory.getCurrentSession().get("dao.Mission", id);
			if (instance == null) {
	...
	public List findByExample(Mission instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("dao.Mission")
